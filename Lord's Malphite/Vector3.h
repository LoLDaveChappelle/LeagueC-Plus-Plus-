...
public:
	...
	///0 = Y, 1 = X
	Vec2 Perpendicular(int offset) const
	{
		if (offset == 0)
		{
			return Vec2(-y, x);
	...
	Vec2 Rotate(float angle) const
	{
		...
		return Vec2(float(((x * cosine) - (y * sine))), float(((y * cosine) + (x * sine))));
	}
};
...
public:
	...
	float DistanceTo(const Vec3 &end) const
	{
		auto dx = end.x - x;
		auto dy = end.y - y;
		auto dz = end.z - z;
	...
	Vec3 Extend(const Vec3& toVector3, float distance) const
	{
		auto from = Vec3(x, y, z);
	...
	Vec3 Perpendicular(int offset) const
	{
		if (offset == 0)
		{
			return Vec3(-y, x, z);
